...
// ProductInfoTableRow 컴퍼넌트의 기능까지 검증 가능
// state, api에 대한 제어 코드를 통합 테스트 대상 컴포넌트로 응집-> 유지보수성 향상, 테스트 단위 나누기 좋음
...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  // 테이블의 행은 row라는 열로 조회 가능
  const [firstItem, secondItem] = screen.getAllByRole('row');

  // 조회한 내용 기준으로 query를 사용할 때는 within 사용
  expect(
...
it('특정 아이템의 수량이 변경되었을 때 값이 재계산되어 올바르게 업데이트 된다', async () => {
  ...
  // placeholder 지정된게 없기 때문에, role와 within으로 조회
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  // queryBy ~~ : 요소의 존재 유무 판단, 존재하지 않아도 에러 X
